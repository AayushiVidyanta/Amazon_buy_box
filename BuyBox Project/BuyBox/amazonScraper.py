		...
		try:
			if nodeID not in loadedURLs:
				...
				try:
					...
					pass
				try:
					_seller = panel.find_element_by_id('merchant-info').text
				except:
					_seller = ''
					pass

				productsFile = open('info/'+nodeID+".tsv", 'w')
				productsFile.write(nodeID + '\t' + _titleName + '\t' + _reviews + '\t' + _rating + '\t' + _producer + '\t' + _price + '\t' + _achoice+ '\t' + _seller)
				productsFile.close()
				loadedOverview = True
					...
					try:
						while 1:
							...
							for row in rows:
								...
								try:
									#star = element.find_element_by_xpath("//p[@class='a-spacing-small']/i[@class='a-icon a-icon-star a-star-4-5']/span[@clas='a-icon-alt']").text
									#m.append(star)
								...
								txt = '. '.join(element.text.split('\n')[:-1])
								if txt != '':
									m.append(txt)
								l.append(m)
								lists.append(l)
							try: 
								nxt = driver.find_element_by_xpath("//div[contains(@class, 'a-text-center a-spacing-large')]/li[contains(@class, 'a-last')]")
								#print(1)
								nxt.click()
							except Exception as e:
								#print(e)
