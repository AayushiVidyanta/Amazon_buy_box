	...
	while queue:
		...
		try:
			if nodeID not in loadedURLs:
				...
				try:
					panel = driver.find_element_by_id('centerCol')
				except:
					panel = driver
				# Overview

				try:
					btn = driver.find_element_by_xpath("//div[@id='mbc']/div[@class='a-box']/div[@class='a-box-inner a-padding-base']/span[@id='mbc-olp-link']/a")
				except:
					continue
				...
				boolVal = False
				try:
					book = driver.find_element_by_xpath("//span[@class='a-list-item'][1]/a[@class='a-link-normal a-color-tertiary']").text
					#print(book)
					if book == 'Books':
						boolVal = True
				except:
				...
				if boolVal == False:
					continue
				try:
					_author = ' '.join(panel.find_element_by_id('bylineInfo').text.split('\n'))
					#print(_author)
					_author = _author.replace('by', '')
					_author = _author.replace('(Author)', '')
				...
				try:
					...
					pass
				try:
					_price = panel.find_element_by_css_selector('.a-size-base.a-color-price.a-color-price').text
				except:
					_price = ''
					pass
				try:
					...
					pass
				try:
					_seller = driver.find_elements_by_xpath("//div[@id='buyNewInner']/div[@class='a-section a-spacing-small']")[1].text.split('\n')[1]
					#print(_seller)
				except Exception as e:
					print(e)
					_seller = ''
				...
				productsFile.write(nodeID + '\t' + _titleName + '\t' + _author + '\t' + _delivery + '\t' + _reviews + '\t' + _rating  + '\t' + _price + '\t' + _achoice + '\t' + _seller)
				productsFile.close()
				loadedOverview = True
						...
						while 1:
							...
							for row in rows:
								...
								try:
									...
									prime = 'Prime : No'
								m.append(prime)
								delivery = driver.find_element_by_class_name("olpShippingInfo").text.split(' ')
								if 'Details' in delivery:
									delivery = delivery[:-1]
								delivery = ' '.join(delivery)
								m.append(delivery)
								pay_on_delivery = element.text.split('\n')[-1]
								if 'Pay on Delivery' in pay_on_delivery:
